  ...
  const getStoreItemObj = (index, store) => {
    const storeItemObj = (
      <StoreItem key={store.seq}>
        ...
        <StoreItemButton onClick={(e)=> {onClickDeleteItem(e, store)}}>삭제</StoreItemButton>
        <StoreItemButton onClick={(e)=> {onClickUpdateItem(e, store)}}>수정</StoreItemButton>
      </StoreItem>
    );
    return storeItemObj;
  }
